...
// We need to inject the render functions right before the return statement.
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            ...
            <select value={selectedCatId} onChange={(e) => setSelectedCatId(e.target.value)} className="w-full bg-white rounded-lg border border-earth/15 px-3 py-3 text-sm focus:outline-none focus:border-phewa text-earth shadow-sm">
              {defaultRooms.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
            </select>
          </div>
          <div>
...
const widgetStart = content.indexOf('<div className="max-w-lg mx-auto relative min-h-[500px]">');
const widgetEnd = content.indexOf('</div>\n            \n            <div className="text-center pt-2 border-t border-earth/10">');

if (widgetStart === -1 || widgetEnd === -1) {
    console.log("Could not find widget bounds");
    process.exit(1);
}
...
const responsiveWidget = `
            ...
            <div className="hidden lg:grid grid-cols-2 gap-10">
              ...
              <div className="h-full">
                {bookingStep === 'calendar' && (
                  <div className={\`transition-opacity duration-300 \${!didCheck ? 'opacity-0 pointer-events-none' : 'opacity-100'}\`}>
                    {renderOptions(false)}
                  </div>
                )}
...
content = content.substring(0, returnStart) + renderFunctions + '\n  ' + content.substring(returnStart, widgetStart) + responsiveWidget + content.substring(widgetEnd);
