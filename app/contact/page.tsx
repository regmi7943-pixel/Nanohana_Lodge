...
export default function ContactPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate standard server dispatch delay
    setTimeout(() => {
  ...
  return (
    <div id="contact-page" className="w-full">
      ...
      <section id="contact-hero" className="relative h-[55vh] min-h-[380px] w-full flex items-center justify-center">
        <Image
          src="https://picsum.photos/seed/teafresh/1600/900"
          alt="Lodge garden entrance"
          fill
          priority
        ...
        <div className="relative z-10 text-center px-5 text-cream max-w-[800px] pt-16">
          ...
          <h1 className="font-serif text-4xl sm:text-5xl font-medium tracking-tight mb-4">
            Come find us in Lakeside.
          </h1>
          <p className="text-cream/90 text-sm max-w-[550px] mx-auto leading-relaxed">
            300 metres from Phewa Lake, directly next to Sahid Chok bus stop, and opposite the {"owner's"} quiet family home.
          </p>
        </div>
      </section>

      {/* TWO-COLUMN CONTACT ELEMENTS */}
      <section id="contact-split" className="bg-cream py-20 text-earth">
        <div className="max-w-[1240px] mx-auto px-5 md:px-10 lg:px-20 grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div id="contact-form-block" className="lg:col-span-7 bg-cream border border-earth/10 rounded-2xl p-6 sm:p-10 shadow-sm space-y-6">
            <h2 className="font-serif text-3xl font-medium mb-2 text-earth">Send us a message</h2>
            <p className="text-xs sm:text-sm text-earth/75 leading-relaxed">
              We coordinate bookings and custom arrangements directly. Standard response turnouts take less than 3 hours.
            </p>
            ...
            {formSubmitted ? (
              <div id="submit-success" className="p-6 rounded-xl bg-forest/10 border border-forest/30 space-y-4 text-center animate-fade-in my-6">
                ...
                <button
                  ...
                  className="px-6 py-2 rounded-full bg-forest text-cream font-sans text-xs font-semibold hover:bg-forest/90 transition-colors"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-4 my-0">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full bg-cream rounded-lg border border-earth/15 px-3 py-2 text-sm focus:outline-none focus:border-phewa text-earth"
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    placeholder="Tell us about your flight time, your trek plans, or extra bedding needs..."
                    required
                    className="w-full bg-cream rounded-lg border border-earth/15 px-3 py-2 text-sm focus:outline-none focus:border-phewa text-earth"
                  />
                </div>
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full py-3 bg-nanohana text-earth font-sans text-sm font-semibold rounded-full hover:bg-nanohana/90 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                  >
          ...
          {/* Right Block - Direct Contact Details and Map Mockup */}
          <div className="lg:col-span-5 space-y-6 w-full">
            <div className="bg-cream border border-earth/10 rounded-2xl p-6 sm:p-8 space-y-4 shadow-sm text-sm">
              <h3 className="font-serif text-xl font-bold text-earth border-b border-earth/10 pb-2">Direct Registry</h3>
              <ul className="space-y-3.5 list-none my-0 pl-0 text-earth/80">
                <li className="flex items-start gap-2.5">
                  ...
                  <span>
                    <strong>Address:</strong> Ammat, Lakeside Street No. 4,<br />
                    Pokhara 33411, Nepal
                  </span>
                </li>
                <li className="flex items-start gap-2.5">
                  ...
                  <span>
                    <strong>Geographics:</strong> <span className="font-mono">28.20403° N, 83.96605° E</span>
                  </span>
                </li>
                <li className="flex items-start gap-2.5">
                  <Phone className="w-5 h-5 text-phewa flex-shrink-0 mt-0.5" />
                  <span>
                    <strong>Mobile / FB:</strong> /NanohanaLodge
                  </span>
                </li>
                <li className="flex items-start gap-2.5">
                  <Clock className="w-5 h-5 text-phewa flex-shrink-0 mt-0.5" />
                  <span>
                    <strong>Coverage:</strong> 24 hour reception & late checkcheckouts
                  </span>
                </li>
                <li className="flex items-start gap-2.5">
                  <Globe2 className="w-5 h-5 text-phewa flex-shrink-0 mt-0.5" />
                  <span>
                    <strong>Languages:</strong> English, Nepali, Hindi, Japanese
                  </span>
                </li>
              </ul>
            </div>

            {/* Embedded maps iframe as per prompt design focused exclusively on GPS */}
            <div id="map-frame" className="relative h-[250px] w-full rounded-2xl overflow-hidden border border-earth/10">
              <iframe
                title="Lakeside Pokhara map"
                className="w-full h-full border-0 grayscale/20 focus:outline-none"
                src="https://maps.google.com/maps?q=28.20403,83.96605&t=&z=14&ie=UTF8&iwloc=&output=embed"
      ...
      {/* THREE WAYS TO ARRIVE SECTION (Forest Canopy bg) */}
      <section id="getting-here-directions" className="bg-forest text-cream py-20 border-t border-white/5">
        <div className="max-w-[1240px] mx-auto px-5 md:px-10 lg:px-20">
          <div className="text-center max-w-[600px] mx-auto mb-16 space-y-1">
            <span className="text-xs font-mono uppercase tracking-[0.15em] text-nanohana font-bold block">
              Simplified Transit
            </span>
            <h2 className="font-serif text-3xl sm:text-4xl text-cream font-medium">Three ways to arrive.</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-sm">
            <div className="p-6 bg-white/5 border border-white/10 rounded-xl space-y-3">
              <Plane className="w-8 h-8 text-nanohana" />
              <h3 className="font-serif text-lg font-bold text-cream">From Pokhara Airport (PKR)</h3>
              <p className="text-cream/80 leading-relaxed">
                The terminal sits between 1.4km and 2.8km away. Taxes from terminals take about 10–15 mins. Direct private paid shuttle transfers can be prearranged securely by informing our front desk lead prior to boarding.
              </p>
            </div>

            <div className="p-6 bg-white/5 border border-white/10 rounded-xl space-y-3">
              <Bus className="w-8 h-8 text-nanohana" />
              <h3 className="font-serif text-lg font-bold text-cream">Arriving By Bus</h3>
              <p className="text-cream/80 leading-relaxed">
                The central Sahid Chok bus stop resides directly adjacent next door to our entrance street. Buses operate connecting Lakeside to downtown Pokhara and surrounding Himalayan routes throughout the day.
              </p>
            </div>

            <div className="p-6 bg-white/5 border border-white/10 rounded-xl space-y-3">
              <Footprints className="w-8 h-8 text-nanohana" />
              <h3 className="font-serif text-lg font-bold text-cream">On Foot From Lakeside</h3>
              <p className="text-cream/80 leading-relaxed">
                From main central Lakeside tracks, walk south past Basundhara Park. Ammat is Lakeside Street No. 4. You will physically notice our lovely flower balconies on your left-hand side. Walk in under 10 minutes.
              </p>
            </div>
          </div>
        </div>
